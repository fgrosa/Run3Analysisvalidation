...
def LoadInputs(inputCfg):
    ...
    if fracMethod not in ["Nb", "fc"]:
        print(
            f"\033[91mERROR: method to subtract nonprompt {fracMethod} not supported. Exit\033[0m"
        )
        sys.exit()
    ...
    if not histos["rawyields"]:
        print(
            f"\033[91mERROR: raw-yield histo {rawYieldHistoName} not found in {rawYieldFileName}. Exit\033[0m"
        )
        sys.exit()
    ...
    if not hNevents:
        print(
            f"\033[91mERROR: normalisation histo {normHistoName} not found in {rawYieldFileName}. Exit\033[0m"
        )
        sys.exit()
    ...
    if not histos["acceffp"]:
        print(
            f"\033[91mERROR: prompt (acc x eff) histo {effPromptHistName} not found in {effFileName}. Exit\033[0m"
        )
        sys.exit()
    ...
    if not histos["acceffnp"]:
        print(
            f"\033[91mERROR: nonprompt (acc x eff) histo {effPromptHistName} not found in {effFileName}. Exit\033[0m"
        )
        sys.exit()
    ...
    for pred in ("central", "min", "max"):
        histos["FONLL"]["nonprompt"][pred] = inFileFONLL.Get(
            f"{FONLLhistoName[channel]}fromBpred_{pred}_corr"
        )
        histos["FONLL"]["nonprompt"][pred].SetDirectory(0)
        if fracMethod == "fc":
            histos["FONLL"]["prompt"][pred] = inFileFONLL.Get(
                f"{FONLLhistoName[channel]}pred_{pred}"
            )
    ...
    norm["sigmaMB"] = (
        normDB["sigma"]["Run2"][system][energy] if observable == "dsigmadpt" else 1.0
    )
...
def Main():
    ...
    hPtSpectrum = TH1F(
        "hPtSpectrum",
        f";{axisTitPt};{axisTitCross}",
        len(ptLims["rawyields"]) - 1,
        ptLims["rawyields"],
    )
    hPtSpectrumWoBR = TH1F(
        "hPtSpectrumWoBR",
        f";{axisTitPt};{axisTitCrossTimesBR}",
        len(ptLims["rawyields"]) - 1,
        ptLims["rawyields"],
    )
    ...
    for iPt, (ptMin, ptMax) in enumerate(
        ...
        crossSecNonPromptFONLL = [
            histos["FONLL"]["nonprompt"][pred].Integral(ptMinFONLL, ptMaxFONLL, "width")
            / (ptMax - ptMin)
        ...
        if fracMethod == "Nb":
            frac = ComputeFractionNb(
                rawY,
                effTimesAccPrompt,
                effTimesAccNonPrompt,
                crossSecNonPromptFONLL,
                ptDelta,
                1.0,
                1.0,
                norm["events"],
                norm["sigmaMB"],
            )  # BR already included in FONLL prediction
        elif fracMethod == "fc":
            crossSecPromptFONLL = [
                histos["FONLL"]["prompt"][pred].Integral(
                    ptMinFONLL, ptMaxFONLL, "width"
                )
                / (ptMax - ptMin)
                for pred in histos["FONLL"]["prompt"]
            ]
            frac = ComputeFractionFc(
                effTimesAccPrompt,
                effTimesAccNonPrompt,
                crossSecPromptFONLL,
                crossSecNonPromptFONLL,
        ...
        gFraction.SetPointError(
            iPt, ptDelta / 2, ptDelta / 2, frac[0] - frac[1], frac[2] - frac[0]
        )
    ...
    # save output file
    outFile = TFile(
        os.path.join(outputDir, f'{cfg["output"]["filename"]}.root'), "recreate"
    )
    hPtSpectrum.Write()
